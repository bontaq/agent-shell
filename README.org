...
Embed file contents in prompts with =@filename= syntax:

- =@file.txt= - Current directory or project file
- =@dir/file= - Relative path
- =@/abs/path= - Absolute path
- =@"file with spaces"= - Quoted filename

Completion triggers after =@=. Supports fuzzy matching with =flex= style (Emacs 27+).
